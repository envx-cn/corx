...
app.get("/", async (c) => {
  ...
  return c.render(<ProfileContent user={user} />, { title: "Profile" });
});

export default app;

// ---------- Page markup (colocated) ----------
function ProfileContent(props: { user: { email: string; via: "access" | "token" } }) {
  const initial = props.user.email[0]?.toUpperCase() ?? "?";
  return (
    <>
      <h1 class="text-3xl font-semibold tracking-tight mb-4">Profile</h1>
      <div class="bg-base-100 border border-base-300 rounded-box p-6 max-w-2xl">
        <div class="flex items-center gap-4">
          ...
          <div class="min-w-0">
            ...
            <div class="text-sm text-base-content/55">
              Signed in via {props.user.via === "access" ? "Cloudflare Access" : "admin token"}
            </div>
          </div>
        </div>
        <dl class="grid sm:grid-cols-2 gap-4 mt-6 text-sm">
          ...
          <div>
            <dt class="text-base-content/55">Auth method</dt>
            <dd class="font-medium capitalize">{props.user.via}</dd>
          </div>
        </dl>
      </div>
      <p class="text-xs text-base-content/50 mt-4 max-w-2xl">
        Console access is controlled by your deployment's Access policy or <code>ADMIN_TOKEN</code>. Changes to the
        account (email, permissions) happen on the upstream identity provider, not here.
      </p>
    </>
  );
}
